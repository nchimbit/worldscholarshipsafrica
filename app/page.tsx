...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/blog-posts')
      .then(res => res.json())
      .then(data => {
        if (data.success) setPosts(data.articles.slice(0, 6))
      })
      .catch(() => {})
  }, [])
    ...
    <main style={{ fontFamily: 'sans-serif', margin: 0, padding: 0 }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', height: '64px', background: '#fff', borderBottom: '1px solid #e0ece4', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 12px rgba(26,107,60,0.07)' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <a href="/login" style={{ padding: '8px 18px', borderRadius: '8px', border: '1.5px solid #1a6b3c', background: 'transparent', color: '#1a6b3c', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>Login</a>
          <a href="/login" style={{ padding: '8px 16px', borderRadius: '8px', background: '#1a6b3c', color: '#fff', border: 'none', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>Sign in with Google</a>
        </div>
      </nav>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0d1f14 0%, #0f3320 50%, #1a5c35 100%)', padding: '80px 40px', display: 'flex', justifyContent: 'center' }}>
        <div style={{ maxWidth: '700px', textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '48px', fontWeight: 900, color: '#fff', lineHeight: 1.1, marginBottom: '20px' }}>Find Your <em style={{ color: '#f0d080', fontStyle: 'italic' }}>Dream</em> Scholarship Worldwide</h1>
          ...
          <div style={{ display: 'flex', background: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.3)' }}>
            <select style={{ padding: '14px 16px', border: 'none', outline: 'none', fontSize: '13px', color: '#6b8a72', background: '#f8faf9', borderRight: '1px solid #e0ece4', minWidth: '130px', cursor: 'pointer' }}>
              <option>All Fields</option>
              <option>Medicine</option>
              <option>Engineering</option>
              <option>Business</option>
            </select>
            <input type="text" placeholder="Search scholarships, universities..." style={{ flex: 1, padding: '14px 16px', border: 'none', outline: 'none', fontSize: '14px', color: '#1a2e1f' }} />
      ...
      <section style={{ padding: '60px 40px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '16px' }}>
          {[['🏥', 'Medicine', '245'], ['⚙️', 'Engineering', '312'], ['💼', 'Business', '198'], ['💻', 'Technology', '267'], ['🎨', 'Arts', '143'], ['⚖️', 'Law', '89']].map(([icon, name, count]) => (
            <a key={name} href="/scholarships" style={{ background: '#f4f7f5', borderRadius: '14px', padding: '20px 14px', textAlign: 'center' as const, cursor: 'pointer', border: '2px solid transparent', textDecoration: 'none', display: 'block' }}>
      ...
      <section style={{ background: '#f4f7f5', padding: '60px 40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '28px' }}>
            <div>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '32px', fontWeight: 700, marginBottom: '8px', color: '#1a2e1f' }}>Latest <span style={{ color: '#1a6b3c' }}>Scholarships</span></h2>
              <p style={{ color: '#6b8a72', fontSize: '14px' }}>Updated daily — auto-fetched from top universities worldwide</p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {[
              ...
              <div key={s.title} style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e8f0eb', boxShadow: '0 2px 12px rgba(26,107,60,0.04)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderTop: '1px solid #f0f4f1' }}>
                  ...
                  <button style={{ padding: '8px 18px', background: '#1a6b3c', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>Apply Now</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1a6b3c, #0f4a28)', padding: '50px 40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '20px', textAlign: 'center' as const }}>
      ...
      {posts.length > 0 && (
        <section style={{ padding: '60px 40px', maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '28px' }}>
            <div>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '32px', fontWeight: 700, marginBottom: '8px', color: '#1a2e1f' }}>Latest <span style={{ color: '#1a6b3c' }}>Blog Posts</span></h2>
              <p style={{ color: '#6b8a72', fontSize: '14px' }}>Complete scholarship guides published today</p>
            </div>
            <a href="/blog" style={{ color: '#1a6b3c', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>View All Posts →</a>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {posts.map((post, i) => (
              <a key={i} href="/blog" style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e0ece4', textDecoration: 'none', display: 'block', boxShadow: '0 2px 12px rgba(26,107,60,0.04)' }}>
                <div style={{ background: 'linear-gradient(135deg,#0d1f14,#1a5c35)', padding: '20px' }}>
                  <div style={{ display: 'flex', gap: '6px', marginBottom: '10px', flexWrap: 'wrap' as const }}>
                    <span style={{ padding: '3px 10px', borderRadius: '100px', fontSize: '10px', fontWeight: 700, background: '#e8f5ee', color: '#1a6b3c' }}>{post.funding}</span>
                    <span style={{ padding: '3px 10px', borderRadius: '100px', fontSize: '10px', fontWeight: 700, background: 'rgba(255,255,255,0.15)', color: '#fff' }}>{post.country}</span>
                  </div>
                  <h3 style={{ fontFamily: 'Georgia,serif', fontSize: '15px', fontWeight: 700, color: '#fff', lineHeight: 1.4 }}>{post.title.split('—')[0]}</h3>
                </div>
                <div style={{ padding: '16px 20px' }}>
                  <p style={{ fontSize: '12px', color: '#6b8a72', lineHeight: 1.6, marginBottom: '12px' }}>{post.metaDescription?.substring(0, 100)}...</p>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '10px', borderTop: '1px solid #f0f4f1' }}>
                    <div>
                      <div style={{ fontSize: '10px', color: '#6b8a72' }}>📅 {new Date(post.generatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                    </div>
                    <span style={{ padding: '6px 14px', background: '#1a6b3c', color: '#fff', borderRadius: '8px', fontSize: '11px', fontWeight: 600 }}>Read More →</span>
                  </div>
                </div>
              </a>
            ))}
      ...
      {/* FOOTER */}
      <footer style={{ background: '#0d1f14', color: 'rgba(255,255,255,0.6)', padding: '40px', textAlign: 'center' as const }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '16px', flexWrap: 'wrap' as const }}>
        ...
        <p style={{ fontSize: '13px', marginBottom: '8px' }}>Africa&apos;s most comprehensive scholarship platform</p>
