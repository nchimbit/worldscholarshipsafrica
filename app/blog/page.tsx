...
const EXTRA_SCHOLARSHIPS: Article[] = [
  { title: 'Australia Awards Scholarships for Africa 2026', slug: 'australia-awards-africa', content: '', link: 'https://australiaawards.gov.au', metaTitle: 'Australia Awards Scholarships Africa 2026', university: 'Australian Government', country: 'Australia', field: 'Any Field', level: 'Masters', funding: 'Fully Funded', deadline: 'May 1, 2026', wordCount: 950, metaDescription: 'Complete guide to Australia Awards Scholarships for African students. Learn eligibility, documents and application tips for this fully funded scholarship.', generatedAt: new Date().toISOString() },
  { title: 'Erasmus+ Programme for African Students 2026', slug: 'erasmus-africa', content: '', link: 'https://erasmus-plus.ec.europa.eu', metaTitle: 'Erasmus+ Scholarships Africa 2026', university: 'European Union', country: 'Europe', field: 'Any Field', level: 'Masters', funding: 'Fully Funded', deadline: 'Feb 1, 2026', wordCount: 980, metaDescription: 'Complete guide to Erasmus+ scholarships for African students to study in Europe. Full funding including tuition, living costs and travel.', generatedAt: new Date().toISOString() },
  { title: 'Korean Government Scholarship Program 2026', slug: 'korean-government-scholarship', content: '', link: 'https://niied.go.kr', metaTitle: 'Korean Government Scholarship Africa 2026', university: 'Korean Government', country: 'South Korea', field: 'Any Field', level: 'Masters/PhD', funding: 'Fully Funded', deadline: 'Mar 15, 2026', wordCount: 920, metaDescription: 'Complete guide to Korean Government Scholarship for African students. Covers full tuition, living allowance and Korean language training.', generatedAt: new Date().toISOString() },
  { title: 'Japanese MEXT Scholarship for Africans 2026', slug: 'mext-japan-africa', content: '', link: 'https://mext.go.jp', metaTitle: 'Japanese MEXT Scholarship Africa 2026', university: 'Japanese Government', country: 'Japan', field: 'Any Field', level: 'Masters/PhD', funding: 'Fully Funded', deadline: 'Apr 20, 2026', wordCount: 940, metaDescription: 'Complete guide to Japanese MEXT Government Scholarship for African students. Full funding plus Japanese language training included.', generatedAt: new Date().toISOString() },
  { title: 'Swiss Government Excellence Scholarships 2026', slug: 'swiss-excellence-africa', content: '', link: 'https://sbfi.admin.ch', metaTitle: 'Swiss Government Excellence Scholarships 2026', university: 'Swiss Government', country: 'Switzerland', field: 'Research', level: 'PhD/Postdoc', funding: 'Fully Funded', deadline: 'Nov 15, 2025', wordCount: 960, metaDescription: 'Complete guide to Swiss Government Excellence Scholarships for African researchers and PhD students. Full funding at Swiss universities.', generatedAt: new Date().toISOString() },
  { title: 'Commonwealth Scholarship for African Students 2026', slug: 'commonwealth-africa', content: '', link: 'https://cscuk.fcdo.gov.uk', metaTitle: 'Commonwealth Scholarship Africa 2026', university: 'Commonwealth Secretariat', country: 'UK', field: 'Any Field', level: 'Masters/PhD', funding: 'Fully Funded', deadline: 'Dec 15, 2025', wordCount: 970, metaDescription: 'Complete guide to Commonwealth Scholarships for African students to study in the UK. Covers tuition, living costs and airfare.', generatedAt: new Date().toISOString() },
]
...
export default function BlogPage() {
  ...
  const filtered = activeCategory === 'All' ? articles : articles.filter(a => a.country === activeCategory)
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px', display: 'grid', gridTemplateColumns: '1fr 280px', gap: '32px' }}>
          <div>
            <div style={{ background: 'linear-gradient(135deg,#0d1f14,#1a5c35)', borderRadius: '16px', padding: '32px', marginBottom: '32px' }}>
              ...
              <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' as const }}>
                ...
                <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)' git commit -m "Fix blog to fetch from database with real dates"}}>📅 📅 Published: {new Date(selected.generatedAt).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} </span>
              </div>
            </div>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', border: '1px solid #e0ece4' }}>
              {selected.content ? selected.content.split('\n').map((line, i) => {
                ...
                if (line.startsWith('- ')) return <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}><span style={{ color: '#1a6b3c', fontWeight: 700 }}>•</span><span style={{ fontSize: '14px', color: '#333', lineHeight: 1.7 }}>{line.replace('- ', '')}</span></div>
                ...
                <div>
                  ...
                  <p style={{ fontSize: '14px', color: '#333', lineHeight: 1.8 }}>Visit the official website to start your application before the deadline of {selected.deadline}.</p>
                </div>
              )}
              <div style={{ marginTop: '32px', padding: '20px', background: '#e8f5ee', borderRadius: '12px', textAlign: 'center' as const }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ background: '#fff', borderRadius: '12px', padding: '18px', border: '1px solid #e0ece4' }}>
              ...
              <a href={selected.link || '#'} target="_blank" rel="noopener noreferrer" style={{ display: 'block', padding: '10px', background: '#1a6b3c', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: 600, textDecoration: 'none', textAlign: 'center' as const, marginTop: '8px' }}>Apply Now →</a>
            </div>
            <div style={{ background: '#fff', borderRadius: '12px', padding: '18px', border: '1px solid #e0ece4' }}>
              ...
              {articles.filter(a => a.slug !== selected.slug).slice(0, 5).map(a => (
                <div key={a.slug} onClick={() => { setSelected(a); window.scrollTo(0, 0) }} style={{ cursor: 'pointer', padding: '10px', borderRadius: '8px', marginBottom: '8px', background: '#f4f7f5' }}>
                  <div style={{ fontSize: '12px', fontWeight: 600, color: '#1a2e1f', marginBottom: '4px', lineHeight: 1.3 }}>{a.title.split('2026')[0]}</div>
    ...
    <main style={{ fontFamily: 'sans-serif', background: '#f4f7f5', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#0d1f14,#1a5c35)', padding: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'inline-block', padding: '4px 14px', background: 'rgba(201,168,76,0.2)', border: '1px solid rgba(201,168,76,0.4)', borderRadius: '100px', color: '#f0d080', fontSize: '11px', fontWeight: 600, marginBottom: '12px' }}>Updated Daily — {today}</div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 40px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '28px', flexWrap: 'wrap' as const }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => { setActiveCategory(cat); setVisibleCount(6) }} style={{ padding: '7px 16px', borderRadius: '100px', border: '1.5px solid #e0ece4', background: activeCategory === cat ? '#1a6b3c' : '#fff', color: activeCategory === cat ? '#fff' : '#6b8a72', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}>
              {cat} ({cat === 'All' ? articles.length : articles.filter(a => a.country === cat).length})
            </button>
          ))}
        </div>

        {loading ? (
          <div style={{ textAlign: 'center' as const, padding: '60px', color: '#6b8a72' }}>
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '24px', marginBottom: '32px' }}>
              {visible.map((article, idx) => (
                <div key={article.slug} onClick={() => { setSelected(article); window.scrollTo(0, 0) }} style={{ background: '#fff', borderRadius: '16px', overflow: 'hidden', border: '1px solid #e0ece4', cursor: 'pointer', boxShadow: '0 2px 12px rgba(26,107,60,0.04)', position: 'relative' as const }}>
                  {idx < 3 && <div style={{ position: 'absolute' as const, top: '12px', right: '12px', background: '#e05c2a', color: '#fff', fontSize: '9px', fontWeight: 700, padding: '2px 8px', borderRadius: '100px' }}>Today</div>}
            ...
            {visibleCount < filtered.length && (
              <div style={{ textAlign: 'center' as const, marginBottom: '40px' }}>
                <button onClick={loadMore} disabled={loadingMore} style={{ padding: '14px 40px', background: loadingMore ? '#6b8a72' : '#1a6b3c', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: loadingMore ? 'not-allowed' : 'pointer', fontFamily: 'sans-serif' }}>
            ...
            {visibleCount >= filtered.length && filtered.length > 0 && (
              <div style={{ textAlign: 'center' as const, padding: '20px', color: '#6b8a72', fontSize: '13px', marginBottom: '40px' }}>
                ✅ You have seen all {filtered.length} articles!
              </div>
            )}
          </>
        )}
      ...
      <footer style={{ background: '#0d1f14', color: 'rgba(255,255,255,0.6)', padding: '30px 40px', textAlign: 'center' as const }}>
